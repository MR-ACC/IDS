...
//数据流头信息
typedef struct tagStreamHeadInfo_t
{
	unsigned int		dwSize;				//本就够大小
	unsigned int		dwStreamNum;		//码流数	
	struct
	{
		//视频
		BOOL				bVideo;				//是否带视频
		...
		int					iWidth;				//视频宽
		int					iHeight;			//视频高
		unsigned int		dwDisplayScale;		//显示比例*1000
...
typedef struct tmOsdRectInfo_t
{
	unsigned int	left;					/*矩形左上角x*/
	unsigned int	top;					/*矩形左上角y*/
	unsigned int	right;					/*矩形宽*/
	unsigned int	bottom;					/*矩形高*/
	BYTE			R;						/*矩形颜色r*/
	BYTE			G;						/*矩形颜色g*/
	BYTE			B;						/*矩形颜色b*/
	BYTE			weight;					/*矩形线宽，绘制时占的像素数 weight>=1,weight==0时默认为1*/
}tmOsdRectInfo_t;

/*
	在YUV中绘制文字及矩形区域相关信息，如要停止绘制请将pText=0、byRectCount=0;
*/
typedef struct tmDrawOsdInfo_t
{
	char*					pText;			/*要绘制的文字*/
	unsigned int			uX;				/*绘制文字的左上角x*/
	unsigned int			uY;				/*绘制文字的左上角y*/
	BYTE					byStep;			/*每字符步进*/
	BYTE					byZoom;			/*字符按byZoom倍放大绘制，byZoom>=1，byZoom==0时默认为1*/
	BYTE					byR;			/*字符颜色r*/
	BYTE					byG;			/*字符颜色g*/
	BYTE					byB;			/*字符颜色b*/
	tmOsdRectInfo_t			*pRect;			/*要绘制的矩形区域*/
	BYTE					byRectCount;	/*要绘制的矩形区域的数量*/
}tmDrawOsdInfo_t;

/*远程文件访问接口回调结构定义*/
typedef struct tmRemoteFileInfoEx_t
{
	/*本结构大小*/
	unsigned int		dwSize;
...
}tmRemoteFileInfoEx_t;

#define	OTHERCMD_PANOPARAM		0X0012 //设置全景相机的显示控制相关参数
#define PANO_CMD_SETMOUSEPT     1	//送入鼠标位置，以选取视频显示区域，设定选中视频编号
#define PANO_CMD_SETLOCKSIZE    2   //设置是否锁定比例显示
#define PANO_CMD_SETCOLOR	    3   //设置视频子区域的框选颜色及视频区域背景颜色
#define PANO_CMD_SETVIEW		4   //设置是否单视频显示

typedef struct tagOtherPanoParam
{
	unsigned int	dwSize;
	int				iCmdType;				// 操作命令
	union
	{
		/* 命令：PANO_CMD_SETMOUSEPT */ 		
		POINT		pt;						// 鼠标相对位置

		/* 命令：PANO_CMD_SETLOCKSIZE */
		BOOL		bLockSize;				// 设置是否按视频原始比例显示		

		/* 命令：PANO_CMD_SETSINGLEVIEW */
		struct
		{
			POINT	pt;						// 鼠标相对位置
			BOOL	bFullScreen;			// 设置是否但画面显示
		}viewInfo;

		/* 命令：PANO_CMD_SETCOLOR */
		struct
		{
			DWORD	dwRGB;					// 设置框选框的颜色
			DWORD	dwBkRGB;				// 设置视频区域背景颜色
		}colorInfo;

	}option;

	BYTE				byVideoNum;				// 每次返回视频数量 
	POINT				ptVideoSolutionList[8];	// 每次都返回分辨率POINT.X=width POINT.Y=height
}tmOtherPanoParam;
...
#define TMCC_INITTYPE_PLAYREMOTEFILE		0x0B//初始化为播放远程文件SDK句柄(通过回调读取数据)
#define TMCC_INITTYPE_RTPSTREAM				0x0C//初始化为播放RTP包的SDK句柄
#define TMCC_INITTYPE_IMAGEPROCESS			0x0D//初始化为图像处理SDK句柄
#define TMCC_INITTYPE_MASK					0xFF//初始化为SDK句柄掩码

#define OTHERCMD_SETOSDINFOTOYUV			0X000A /*OSD信息到YUV*/
...
//多码流显示回调
typedef int  (CALLBACK* TMCC_MULTISTREAM_CALLBACK)( HANDLE hTmCC, tmMultiStreamInfo_t* pMultiStreamCfg, void* context );
//错误回调
typedef int  (CALLBACK* TMCC_ERRCODE_CALLBACK)(HANDLE hTmCC, int errCode, void* context);

//错误回调
typedef int  (CALLBACK* TMCC_ERRCODE_CALLBACK)(HANDLE hTmCC, int errCode, void* context);

typedef struct tmErrCallBackInfo
{
	int		(CALLBACK* fnErrCallBack)(HANDLE hTmCC, int errCode, void* context);
	void	*pContext;
}tmErrCallBackInfo_t;

/*
 *函  数: TMCC_RegisterErrCodeCallBack 
 *说  明: 注册SDK中错误码输出回调
 *参  数: pCallBack 回调函数指针
 *返回值: 返回0表示成功
 */
TMCC_API unsigned int TMCC_CALL TMCC_RegisterErrCodeCallBack( HANDLE hTmCC, TMCC_ERRCODE_CALLBACK pCallBack, void* context );
 ...
 *参  数: hTmCC为控制句柄，
		  pVoiceInHead用于解码，
		  pVoiceOutHead用于音频采集，通过TMCC_RegisterVoiceDataCallBack注册音频数据回调；
          pVoiceInHead、pVoiceOutHead可以一起使用也可分开使用
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetPlayWaveFile( HANDLE hTmCC, char *pFileName, unsigned char bPlayNum );
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_UpgradeWebOcx( HANDLE hTmCC, const char *lpszName );
 ...
 * 返回值: 成功返回文件头长度，失败返回<=0
 */
TMCC_API int TMCC_CALL TMCC_CreateFileHead( void* pStreamHead, unsigned char* pHeadBuf, int iBufSize );
 ...
 * 返回值: 成功返回文件头长度，失败返回<=0
 */
TMCC_API int TMCC_CALL TMCC_CreateStreamHead( unsigned char* pHeadBuf, int iBufSize, void* pStreamHead );
...
/*
 *函  数: TMCC_RegisterMultiStreamCallBack
 *说  明: 注册解码视频自会输出回调
 *参  数: hTmCC为服务器句柄,pCallBack用户回调函数指针,context为用户自定义指针
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API void TMCC_CALL TMCC_RegisterMultiStreamCallBack( HANDLE hTmCC, TMCC_MULTISTREAM_CALLBACK pCallBack, void* context );
...
*函  数: TMCC_GetBalance
*说  明: 得到左右声道的均衡值
*返回值: 成功返回Balance值，失败负数
*/
TMCC_API int TMCC_CALL TMCC_GetBlance( HANDLE hTmCC );

/*
*函  数: TMCC_GetBalance
*说  明: 设置左右声道的均衡值
*参  数: hTmCC为控制句柄，iBlance为均衡值，可以控制左右声道的播放 取值范围：-10000~10000，-10000左声道，10000右声道
*返回值: 成功返回Balance值，失败负数
*/
TMCC_API int TMCC_CALL TMCC_SetBlance( HANDLE hTmCC, int iBlance );
...
/*
 *函  数: TMCC_SetCurrentAvTrack
 *说  明: 设置当前音视频播放轨道，只对多码流同时解码显示有效
 *参  数: hTmCC为控制句柄，iAvTrack为当前播放的码流
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetCurrentAvTrack( HANDLE hTmCC, int iAvTrack );

/*
 *函  数: TMCC_SetDisplayPos
 *说  明: 读取显示视频的显示位置
 *参  数: hTmCC为控制句柄，iRegionNum为显示号,pSrcRect为显示位置相对于原始图像,hDestWnd显示窗口,bEnable为是否显示
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetDisplayPos( HANDLE hTmCC, int iRegionNum, RECT* lpRect );

/*
 *函  数: TMCC_SetAutoFitWindow
 *说  明: 读取显示视频的显示位置
 *参  数: hTmCC为控制句柄，iRegionNum为显示号,bAutoFitWindow为显示是否自动适应窗口变化
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SetAutoFitWindow( HANDLE hTmCC, int iRegionNum, BOOL bAutoFitWindow );
...
//////////////////////////////////////////////////////////////////////////////////////
/*
 *函  数: TMCC_OpenImgFile
 *说  明: 输入BMP、JPG格式图像文件用于显示、图像处理
 *参  数: hTmCC为控制句柄
 *		  pFileName 图像文件名称	
 *		  hDisplayWnd 显示图像的窗口句柄
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_OpenImgFile( HANDLE hTmCC, const char* pFileName, HWND hDisplayWnd );

/*
 *函  数: TMCC_CloseImgFile
 *说  明: 关闭图像
 *参  数: hTmCC为控制句柄
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_CloseImgFile(HANDLE hTmCC);
 ...
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_GetImgSize(HANDLE hTmCC, int &width, int &height);

/*
 *函  数: TMCC_ShowImage
 *说  明: 显示图像文件
 *参  数: hTmCC为控制句柄，bShowUnprocessedImg为TRUE时显示原图，bShowUnprocessedImg为FALSE时显示处理后图像
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_ShowImage(HANDLE hTmCC, BOOL bShowUnprocessedImg);

/*
 *函  数: TMCC_ImgEnhancement
 *说  明: 图像增强函数
 *参  数: hTmCC					为控制句柄，
 *        dLevel				(0.1-20)    默认为3
 *        iRadius				(1-255)     默认30
 *        dEps					(0.001-0.5) 默认0.005
 *        x、y、width、height	从图像中左上角(x,y)处开始处理宽为width、高为height的矩形区域 其中
 *                              if (width <=0)	{width  = m_iImageWidth-x;}
 *                              if (height<=0)	{height = m_iImageHeight-y;}
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_ImgEnhancement(HANDLE hTmCC, double dLevel, int iRadius, double dEps, int x, int y, int width, int height);

/*
 *函  数: TMCC_SaveProcessedImgFile
 *说  明: 保存处理后图像
 *参  数: hTmCC为控制句柄 pFileName保存文件名
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_SaveProcessedImgFile(HANDLE hTmCC, const char* pFileName);

/*
 *函  数: TMCC_GetOriginalImgData
 *说  明: 获得图像的RGB24数据的起始指针 及 RGB24数据大小
 *参  数: hTmCC为控制句柄 
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_GetOriginalImgData(HANDLE hTmCC, BYTE*& pData, int& iDataSize);

/*
 *函  数: TMCC_GetOriginalImgData
 *说  明: 获得处理后图像的RGB24数据的起始指针 及 RGB24数据大小
 *参  数: hTmCC为控制句柄 
 *返回值: 成功返回TMCC_ERR_SUCCESS，失败返回其它值
 */
TMCC_API int TMCC_CALL TMCC_GetResultImgData(HANDLE hTmCC, BYTE*& pData, int& iDataSize);
